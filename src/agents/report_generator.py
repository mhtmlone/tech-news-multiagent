...
class ReportGeneratorAgent(BaseAgent):
    ...
    async def process(self, input_data: dict) -> GeneratedReport:
        ...
        executive_summary = llm_report_data.get(
            "executive_summary"
        ) or self._generate_template_summary(articles, technologies)
    ...
    def _build_sections_from_llm_data(
        ...
        sections.append(
            ReportSection(title="Technology Trends", content=trends_content, priority=1)
        )

        # Key Developments Section - template-based (no LLM needed)
        developments_content = self._generate_developments_section(
            technologies, developments
        )
        sections.append(
            ReportSection(
                title="Key Developments", content=developments_content, priority=2
            )
        )
        ...
        sections.append(
            ReportSection(
                title="Companies in Focus", content=companies_content, priority=3
            )
        )
        ...
        sections.append(
            ReportSection(title="Geographic Insights", content=geo_content, priority=4)
        )
        ...
        sections.append(
            ReportSection(
                title="Significance Analysis", content=significance_content, priority=5
            )
        )
    ...
    def _build_significance_section_from_llm_data(
        self, llm_data: dict, articles: list[dict], technologies: list
    ) -> str:
    ...
    def _build_significance_from_llm_data(
        ...
        result = {
            ...
            "high_relevance_count": len(
                [a for a in articles if a.get("relevance_score", 0) > 0.5]
            ),
            "average_sentiment": sum(a.get("sentiment_score", 0) for a in articles)
            / len(articles)
            if articles
            else 0,
            "top_articles": [
    ...
    def _generate_template_summary(
        self, articles: list[dict], technologies: list
    ) -> str:
        ...
        if top_sources:
            summary += (
                f"The primary sources were {', '.join([s[0] for s in top_sources])}. "
            )
    ...
    def _generate_developments_section(
        self, technologies: list, developments: dict
    ) -> str:
        ...
        if promising:
            ...
            for p in promising[:5]:
                content += (
                    f"- **{p.get('tech_name', 'Unknown')}**: {p.get('reason', 'N/A')}\n"
                )
    ...
    def _generate_template_significance(self, articles: list[dict]) -> str:
        ...
        for article in articles:
            ...
            content_preview = (
                article_content[:300] if article_content else "No content available"
            )
    ...
    def _format_companies(
        self, companies: list[dict], articles: list[dict]
    ) -> list[dict]:
    ...
    def _format_markdown(self, report: GeneratedReport) -> str:
        ...
        if report.significance_analysis:
            ...
            lines.append(
                f"- **Total Articles Analyzed:** {sa.get('total_articles', 0)}"
            )
            lines.append(
                f"- **High Relevance Articles:** {sa.get('high_relevance_count', 0)}"
            )
            lines.append(
                f"- **Average Sentiment:** {sa.get('average_sentiment', 0):.2f}"
            )
            lines.append("")
