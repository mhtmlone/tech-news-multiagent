...
class EntityExtractor:
    ...
    def _extract_context(
        self, text: str, entity: str, context_length: int = 100
    ) -> str:
    ...
    async def extract_all_with_llm(self, text: str) -> dict:
        ...
        if self.use_llm and self.llm_analyzer:
            try:
                llm_results = await self.llm_analyzer.extract_entities_with_context(
                    text
                )

                # Merge results, preferring LLM for context
                seen_companies = {
                    c["name"].lower() for c in pattern_results["companies"]
                }
                seen_countries = {
                    c["name"].lower() for c in pattern_results["countries"]
                }
    ...
    def get_companies_by_country(self, country: str) -> list[str]:
        ...
        return [
            name
            for name, c in self.TECH_COMPANIES.items()
    ...
    async def extract_all_unified(
        ...
        if self.use_llm and self.llm_analyzer:
            try:
                ...
                seen_companies = {
                    c["name"].lower() for c in llm_results.get("companies", [])
                }
                ...
                # Merge countries
                seen_countries = {
                    c["name"].lower() for c in llm_results.get("countries", [])
                }
                ...
                # Merge technologies
                seen_technologies = {
                    t["name"].lower() for t in llm_results.get("technologies", [])
                }
                ...
                logger.warning(
                    f"LLM unified extraction failed, falling back to patterns: {e}"
                )
