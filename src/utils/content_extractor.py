...
async def fetch_article_content(
    ...
    if html_content and html_content != HARD_BLOCKED:
        ...
        content = (
            extract_with_trafilatura(url, html_content)
            or extract_with_newspaper3k(url)
            or extract_with_readability(html_content)
            or extract_with_beautifulsoup(html_content)
        )
    ...
    if playwright_html:
        content = (
            extract_with_trafilatura(url, playwright_html)
            or extract_with_readability(playwright_html)
            or extract_with_beautifulsoup(playwright_html)
        )
...
async def _fetch_html_content(
    ...
    try:
            ...
            if response.status == 200:
                ...
                if failure_logger:
                    failure_logger.log_rate_limit(
                        url=url, retry_after=response.headers.get("Retry-After")
                    )
        ...
        if failure_logger:
            failure_logger.log_network_timeout(url=url, operation="fetch_html")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if not any(indicator in html_content for indicator in waf_indicators):
                    logger.info(f"Playwright successfully bypassed WAF for {url}")
                    return html_content
                else:
                    logger.warning(f"Playwright could not bypass WAF for {url}")
                    return ""

            except Exception as e:
                logger.warning(f"Playwright page navigation failed for {url}: {e}")
                return ""
            finally:
                await browser.close()

    except ImportError:
        logger.debug("Playwright not installed, skipping headless browser fallback")
    except Exception as e:
...
def extract_with_beautifulsoup(html_content: str) -> str:
    ...
    try:
        ...
        content = (
            extract_from_article_tag(soup)
            or extract_from_main_tag(soup)
            or extract_from_content_div(soup)
            or extract_from_schema_org(soup)
            or extract_from_meta_tags(soup)
            or extract_from_paragraphs(soup)
            or extract_from_readability_style(soup)
        )
...
def extract_from_content_div(soup: BeautifulSoup) -> str:
    ...
    content_patterns = [
        {"class_": re.compile(r"article|content|post|entry|story|body|text", re.I)},
        {"id": re.compile(r"article|content|post|entry|story|body|text", re.I)},
        {
            "class_": re.compile(
...
def extract_from_readability_style(soup: BeautifulSoup) -> str:
    ...
    text_elements = soup.find_all(["p", "div", "section", "article", "span"])
    ...
    for element in text_elements:
        ...
        if class_attr:
            ...
            if any(
                kw in class_str
                for kw in ["content", "article", "post", "story", "body", "text"]
            ):
                score += 500
        ...
        if id_attr and any(
            kw in id_attr.lower()
            for kw in ["content", "article", "post", "story", "body"]
        ):
            score += 500
